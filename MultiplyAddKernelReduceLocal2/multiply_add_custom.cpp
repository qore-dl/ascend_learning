...
class KernelMultiplyAdd{
    ...
    __aicore__ inline uint32_t Compute_GM_OUT_REQUIRED_Byte(uint32_t raw_element_num,uint32_t block_element_scale){
        uint32_t out_size = (uint32_t)((raw_element_num + block_element_scale - 1)/block_element_scale)*DATA_BLOCK_SIZE;
        ...
        uint32_t totalLength,uint32_t tileNum,uint32_t element_size,uint32_t lower_sync_size)
    {
        ...
        if(AscendC::GetBlockIdx() < 1){
            zGm.SetGlobalBuffer((__gm__ half *)z, (block_element_scale)*AscendC::GetBlockNum());
        }else{
            // tileNum*BUFFER_NUM*
            // tileNum*BUFFER_NUM*
            zGm.SetGlobalBuffer((__gm__ half *)z + ((block_element_scale) * AscendC::GetBlockIdx()), 
            (block_element_scale));
        }
        // *block_element_scale
        uint32_t raw_element_num = tileNum*BUFFER_NUM*AscendC::GetBlockNum();
        uint32_t required_out_gm_size = Compute_GM_OUT_REQUIRED_Byte(raw_element_num,this->block_element_scale);
        uint32_t required_element_scale{required_out_gm_size/element_size};

        // (tileNum*BUFFER_NUM)*AscendC::GetBlockNum()
        
        // zGm.SetGlobalBuffer((__gm__ half *)z, required_element_scale);
        // sumGm.SetGlobalBuffer((__gm__ half *)z, required_element_scale);
        ...
        pipe.InitBuffer(vecInSync, BUFFER_NUM, 64 * sizeof(int32_t));

        // pipe.InitBuffer(inTotalReduceQueueZ,BUFFER_NUM,(tileNum*BUFFER_NUM*block_element_scale) * sizeof(half));
        // pipe.InitBuffer(inTotalReduceQueueZ,BUFFER_NUM,(tileNum*BUFFER_NUM*block_element_scale) * sizeof(half));
        //required_out_gm_size
        // pipe.InitBuffer(inTotalReduceQueueZ,BUFFER_NUM,tileNum*BUFFER_NUM*AscendC::GetBlockNum() * sizeof(uint16_t));
    ...
    __aicore__ inline void Process()
    {
        ...
        for (size_t i{0}; i < loopCount; i++) {
                CopyIn(i);
                Compute_ma(i,&local_sum);
                // CopyOut(i);
        }

        // this->blockIdx*loopCount+
        ...
        // uint32_t total_Wrote_byte = total_loopCount * this->element_size;
        // uint32_t total_occupy_byte = Compute_GM_OUT_REQUIRED_Byte(total_loopCount,this->block_element_scale);
        if(AscendC::GetBlockIdx() < 1)
        {
            // AscendC::LocalTensor<half> tk_zlocal = inTotalReduceQueueZ.AllocTensor<half>();

            // AscendC::DataCopy(tk_zlocal,sumGm,total_loopCount);

            // AscendC::LocalTensor<half> zTotalReduce = outReduceQueueZ.AllocTensor<half>();
            // // outReduceQueueZ.AllocTensor<half>();
            // AscendC::LocalTensor<half> zWorkLocal = workLocalSpace.AllocTensor<half>();
            // AscendC::ReduceSum(zTotalReduce,tk_zlocal,zWorkLocal,total_loopCount);
            // // outReduceQueueZ.EnQue<half>(zTotalReduce);
            // workLocalSpace.FreeTensor(zWorkLocal);
            // inTotalReduceQueueZ.FreeTensor(tk_zlocal);

            //  CopyOut();
            // outReduceQueueZ.DeQue<half>
            // vecInSync
            // AscendC::LocalTensor<half> zTotalReduce = calcbufCoreRes.Get<half>();;
            // AscendC::Duplicate<half>(zTotalReduce, 0, this->block_element_scale);
            ...
            // zTotalReduce = outReduceQueueZ.DeQue<half>();
            // zGm.SetValue(0,zTotalReduce.GetValue(0));
            // outReduceQueueZ.FreeTensor(zTotalReduce);
            // * this->block_element_scale
            //  loopCount * 
            float sum = 0;
            for(int i = 0; i < AscendC::GetBlockNum(); ++i) {
    ...
    // AscendC::GlobalTensor<half> sumGm;
    ...
    // __aicore__ inline void CopyInterCoreIn(int32_t total_comp)
    // {
    //     AscendC::LocalTensor<half> tk_zlocal = inTotalReduceQueueZ.AllocTensor<half>();
    //     // AscendC::LocalTensor<half> yLocal = inQueueY.AllocTensor<half>();
    ...
    //     // [progress_id*this->block_element_scale]
    //     AscendC::DataCopy(tk_zlocal,zGm,
    //         this->block_element_scale);
    ...
    __aicore__ inline void Compute_ma(size_t progress,float* local_reduce)
    {
        ...
        *local_reduce += (float)z_acc.GetValue(0);
         
        // outReduceQueueZ.EnQue<half>(z_acc);
        inQueueX.FreeTensor(xLocal);
        inQueueY.FreeTensor(yLocal);
        outReduceQueueZ.FreeTensor(z_acc);
    }
    ...
    //     inTotalReduceQueueZ.FreeTensor(bk_zlocal);
    //     // outReduceQueueZ.EnQue<half>(zTotalReduce);
    // }
    ...
    __aicore__ inline void CopyOut(int32_t progress)
    {
        ...
        // progress * this->block_element_scale
        // progress
        zGm.SetValue(progress * this->block_element_scale, zLocal.GetValue(0));
        outReduceQueueZ.FreeTensor(zLocal);
    }

    __aicore__ inline void StoreLocal(float& local_reduce)
    {
        // AscendC::LocalTensor<half> zLocal = outReduceQueueZ.DeQue<half>();
        // progress * this->block_element_scale
        // progress
        // progress * this->block_element_scale
        zGm.SetValue(0, (half)(local_reduce));
        // outReduceQueueZ.FreeTensor(zLocal);
    }
